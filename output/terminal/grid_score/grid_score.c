/*
  ...
  Tetris-Grid/-Score mit mcurses via MQTT
  =======================================
          Uwe Berger, 2108
  
 
  Tetris-Spielfeld/-staende, Daten via MQTT
...
void mqtt_grid_callback(struct mosquitto *mosq, void *userdata, const struct mosquitto_message *message)
{
    ...
    if (strcmp(topics[topic_count-1], "create_game_screen") == 0) {
		...
		attrset(A_NORMAL);
		mvaddstr(SCORE_Y,    SCORE_X, "Level...:");
		mvaddstr(SCORE_Y+2,  SCORE_X, "Bricks..:");
		mvaddstr(SCORE_Y+4,  SCORE_X, "Lines...:");
		mvaddstr(SCORE_Y+6,  SCORE_X, "Points..:");
		mvaddstr(SCORE_Y+8,  SCORE_X, "Time [s]:");
		mvaddstr(SCORE_Y+13, SCORE_X, "'q' --> Exit");
	}
    ...
    if (strcmp(topics[topic_count-1], "time") == 0) {
		...
		mvaddstr(SCORE_Y+8, SCORE_X+10, buf);
	}

    if (strcmp(topics[topic_count-1], "level") == 0) {
		sprintf(buf, "%8s", (char *)message->payload);
		mvaddstr(SCORE_Y, SCORE_X+10, buf);
	}

    if (strcmp(topics[topic_count-1], "bricks") == 0) {
		sprintf(buf, "%8s", (char *)message->payload);
		mvaddstr(SCORE_Y+2, SCORE_X+10, buf);
	}

    if (strcmp(topics[topic_count-1], "lines") == 0) {
		sprintf(buf, "%8s", (char *)message->payload);
		mvaddstr(SCORE_Y+4, SCORE_X+10, buf);
	}

    if (strcmp(topics[topic_count-1], "points") == 0) {
		sprintf(buf, "%8s", (char *)message->payload);
		mvaddstr(SCORE_Y+6, SCORE_X+10, buf);
	}

    if (strcmp(topics[topic_count-1], "gamestatus") == 0) {
		switch(atoi(message->payload)) {
			case 0:		// ...laeuft
				mvaddstr(SCORE_Y+10, SCORE_X, "          ");
				break;
			case 1:		// ...Pause
				attrset(A_REVERSE);
				mvaddstr(SCORE_Y+10, SCORE_X, "Pause...  ");
				attrset(A_NORMAL);
				break;
			case 2:		// ...Game over
				attrset(A_REVERSE);
				mvaddstr(SCORE_Y+10, SCORE_X, "Game over!");
				attrset(A_NORMAL);
				break;
		}
	}
